...
function p1(d, logo, W, H) {
  ...
  fill(d, '#3CB4FF'); d.rect(0, 0, W * 0.35, H * 0.35, 'F');
  fill(d, '#D21414'); d.rect(W * 0.65, H * 0.65, W * 0.35, H * 0.35, 'F');
  ...
  d.setLineWidth(0.1); stroke(d, '#3CB4FF');
  ...
  // Cuadros rotados
  d.setLineWidth(0.2); stroke(d, '#3CB4FF');
  d.setGState(new d.GState({ opacity: 0.04 }));
  ...
  // Esquinas
  d.setLineWidth(0.8); stroke(d, '#3CB4FF');
  ...
  fill(d, '#D21414'); d.rect(W / 2, H - 3, W / 2, 3, 'F');
  d.setGState(new d.GState({ opacity: 1 }));
}
...
function p2(d, svcCount, W, H) {
  ...
  stroke(d, '#3CB4FF'); d.setLineWidth(0.2); d.line(18, 0, 18, H);
...
function p4(d, W, H) {
  ...
  // Shapes
  d.setGState(new d.GState({ opacity: 0.03 }));
  fill(d, '#3CB4FF'); d.rect(20, 20, 40, 40, 'F');
  fill(d, '#D21414'); d.rect(W - 60, H - 60, 35, 35, 'F');
  d.setGState(new d.GState({ opacity: 1 }));
...
function p5(d, W, H) {
  ...
  const items = [
    ...
    { t: 'SOPORTE 24/7', d: 'Acompanamiento constante tecnico.', c: '#D21414' },
    { t: 'CALIDAD GARANT.', d: 'Mejores materiales y metodologias probadas.', c: '#3CB4FF' },
    { t: 'SOLUCION INTEGRAL', d: 'Centralizamos multiples requerimientos.', c: '#D21414' },
  ];
...
function pSvc(d, svcs, logo, cache, idx, W, H) {
  ...
  svcs.forEach((s, i) => {
    ...
    stroke(d, '#3CB4FF'); d.setLineWidth(0.15); d.roundedRect(x, y, cw, ch, 4, 4, 'S');
    const ih = ch * 0.5;
  ...
  d.setGState(new d.GState({ opacity: 0.3 }));
  fill(d, '#3CB4FF'); d.rect(0, H - 2, W / 2, 2, 'F');
  fill(d, '#D21414'); d.rect(W / 2, H - 2, W / 2, 2, 'F');
  d.setGState(new d.GState({ opacity: 1 }));
}
